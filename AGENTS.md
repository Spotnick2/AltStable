...
- Before starting a substantive code review, assess its scope and recommend an appropriate model and reasoning effort. Use GPT-5.6 Sol as the minimum baseline for code reviews; do not recommend an older or less capable model family. Route reviews by both review shape and consequence rather than treating the pairs as a strict weakest-to-strongest ladder:
  - `gpt-5.6-sol` / `medium`: routine substantive reviews.
  - `gpt-5.6-sol` / `high`: bounded surfaces with dense logic or subtle localized behavior that needs deeper checking.
  - `gpt-6-astra` / `medium`: broad, ambiguous or cross-component behavior that needs stronger overall judgment.
  - `gpt-6-astra` / `high`: work that is both broad and high-consequence, such as security/privacy boundaries, destructive changes, concurrency, identity or silent-corruption risks.
- The routing matrix is project policy informed by OpenAI's general model guidance, not an official OpenAI code-review matrix. Sol/medium is the baseline because medium is Sol's documented default and, as verified in OpenAI's release notes on 2026-09-21, Sol powers Codex cloud code review for eligible customers. Sol/high and Astra/medium solve different review shapes; do not describe Astra/medium as necessarily cheaper than Sol/high without project measurements.
- Treat `gpt-5.6-terra` / `medium` for documentation-only, generated or obviously mechanical reviews as an experimental cost optimization, not an approved route. Add it only after evaluation against representative historical reviews shows that it preserves required review accuracy and the owner approves the route.
- Dedicated vulnerability assessments using the Codex Security scan workflow are separate from ordinary PR correctness reviews. For that workflow, recommend `gpt-5.6-sol` / `xhigh` as OpenAI documents, but use it only with explicit approval under the cost policy; do not generalize it to ordinary security-sensitive PR reviews.
- Keep making model/effort recommendations on future reviews; the user handles session model switches. Ask when an escalation or de-escalation is warranted, and honor explicit approval to use the current model for that review without asking again. Do not silently switch models or effort. Follow the user's cost policy.
...
- **Check every API a change calls against the client's own API dump**, not memory, a wiki or
  Retail documentation. `C:\Projects\References\forever-api-<build>.md` is the complete surface, generated by the
  client itself: documented signatures with argument order and `optional` markers, events with
  payloads, enums with values, every global, every namespace's functions, and widget methods.
  Use the file whose `<build>` matches `MEASURED_ON_BUILD` in `Config.lua`. If no file matches,
...
- **Absence from the dump is strong evidence, not proof.** A name can live as a global, in a `C_*`
  ...
  system. Three APIs were declared missing on this project by checking one surface only. Say which
  surfaces you checked.
- **Presence is not behaviour.** The dump proves a name exists and what shape it declares, not that
  it works on a Vanilla-content client: `test_cameraOverShoulder` is present, writable, and inert.
- **No persistence claim without a full client exit.** `/reload` keeps the process alive, so
  values survive in memory and look persisted. Every false "it persists" result on this project
  came from `/reload`. Require a real exit, a relaunch, and the value found on disk.
...
- **The API dump is build-stamped — regenerate it when the client bumps.**
  `C:\Projects\References\forever-api-<build>.md` is a snapshot of one build, and a stale one reads
...
- **Code review is the exception** — see "Review preferences" above: Sol/medium is the minimum
  baseline, with Sol/high and Astra routes chosen by review shape and consequence, recommended to
  the owner, and never switched silently.
